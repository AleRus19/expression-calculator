...
public class EvaluateTests {
    ...
    [Test]
    public void Test4() {
        var res = expressionParser.Evaluate("(3 + (4 * 2)) / ((( 1 - 5 ) ^ 2) ^ 3)");

        Assert.That(res, Is.EqualTo((3 + (4 * 2)) / Math.Pow(Math.Pow(1 - 5, 2), 3)));
    }

    [Test]
    public void Test5() {
        var res = expressionParser.Evaluate("5 + (((2 / (3- 8)) ^ 5) ^ 2)");

        Assert.That(res, Is.EqualTo((5 + Math.Pow(Math.Pow(2.0 / (3.0 - 8.0), 5), 2))));
    }
}